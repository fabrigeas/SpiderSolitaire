...
function initGame() {
  ...
  let panel     = document.getElementById("panel"),
      compltd   = document.querySelector(".stack.completed");// a div that contains the stacks of completed cards


  for(let i = config.numberOfStacks; i > -1; i--) {
    panel.appendChild(createStackDOM(i));// create stacks
    compltd.appendChild(document.createElement("div"));// Create stack placeholders for completed card
  }
  
  completedStacks = compltd.children;
...
function startGame(resumeGame) {
  ...
  if(resumeGame) {
    ...
    game.isPlaying = true;
    config.numberOfColors = parseInt(document.querySelector("input[name=numberOfColors]:checked").value);

    //put all the cards in the bank
    CardGenerator(config.numberOfColors).forEach( card => addCardToStack( createCard(card,-1,true), bank));
...
function dragStart(event) {
  ...
  card.style.height = (stackToBeMoved.length * 32) -32 + card.offsetHeight + "px";
  card.classList.add("moving");
  card.parentElement.classList.add("old");

  stackToBeMoved.forEach( card =>  setTimeout(() => card.classList.add('hide')))
}
...
function addCardToStack(card, newStack) {
  ...
  if(! card.classList.contains("hidden") && newStack.classList.value) {
    let lastElementChild = newStack.lastElementChild,
        lastMargin       = lastElementChild ? parseInt(lastElementChild.style.marginTop) : 0;
  ...
  tails.forEach( card => {
    ...
    while(card = card.previousSibling) {
      ...
      if(accumulator.length == config.numberOfcorrectCards) {
        ...
        moveStachToCompleted(accumulator, card.previousElementSibling);
        score += (100 - moves);
 ...
 */
function moveStachToCompleted(stack, card) {
  stack.forEach( card => {
...
function GameTimer() {
  ...
  this.displayTime = () => this.timeDOM.textContent = `${this.beautifyTime(this.h)}:${this.beautifyTime(this.m)}:${this.beautifyTime(this.s)}`;
